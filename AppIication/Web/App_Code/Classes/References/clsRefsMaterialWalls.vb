...
Public Class clsRefsMaterialWalls
...
#End Region
    ...
    Public Function browseRefsMaterialWalls(ByVal _criteria As String) As DataTable
        Dim sql As String = ""
        sql = "SELECT trans_id, ref_desc, ref_points, sort_order, is_active, create_user, create_date, last_user, last_date FROM tbl_refs_material_walls " & _
              "WHERE is_active <> '' ORDER BY ref_desc "
        Return _clsDB.Fill_DataTable(sql, "tbl_refs_material_walls")
    End Function


    Public Sub saveRefsMaterialWalls()
        If transId = "" Then
            With _clsDB.dbUtility
                .fieldItems = "trans_id,ref_desc,ref_points,sort_order,is_active,create_user,create_date"
                .sqlString = .getSQLStatement("tbl_refs_material_walls", "INSERT")
        ...
        Else
            With _clsDB.dbUtility
                ...
                .sqlString = .getSQLStatement("tbl_refs_material_walls", "UPDATE", "trans_id")
                .ADDPARAM_CMD_String("ref_desc", _refDesc)
                .ADDPARAM_CMD_String("ref_points", _refPoints)
                .ADDPARAM_CMD_String("sort_order", _sortOrder)
                .ADDPARAM_CMD_String("is_active", _isActive)
    ...
    Public Sub getRefsMaterialWalls(ByVal _id As String)
        Dim dt As New DataTable
        dt = _clsDB.Fill_DataTable("SELECT * FROM tbl_refs_material_walls WHERE trans_id='" & _id & "'")
