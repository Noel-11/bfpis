...
Public Class clsRefsIsFisherFolk
...
#End Region
    ...
    Public Function browseRefsIsFisherfolk(ByVal _criteria As String) As DataTable
        Dim sql As String = ""
        sql = "SELECT trans_id, ref_desc, ref_points, sort_order, is_active, create_user, create_date, last_user, last_date FROM tbl_refs_is_fisherfolk " & _
              "WHERE is_active <> '' ORDER BY ref_desc "
        Return _clsDB.Fill_DataTable(sql, "tbl_refs_is_fisherfolk")
    End Function


    Public Sub saveRefsIsFisherfolk()
        If transId = "" Then
            With _clsDB.dbUtility
                .fieldItems = "trans_id,ref_desc,ref_points,sort_order,is_active,create_user,create_date"
                .sqlString = .getSQLStatement("tbl_refs_is_fisherfolk", "INSERT")
        ...
        Else
            With _clsDB.dbUtility
                ...
                .sqlString = .getSQLStatement("tbl_refs_is_fisherfolk", "UPDATE", "trans_id")
                .ADDPARAM_CMD_String("ref_desc", _refDesc)
                .ADDPARAM_CMD_String("ref_points", _refPoints)
                .ADDPARAM_CMD_String("sort_order", _sortOrder)
                .ADDPARAM_CMD_String("is_active", _isActive)
    ...
    Public Sub getRefsIsFisherfolk(ByVal _id As String)
        Dim dt As New DataTable
        dt = _clsDB.Fill_DataTable("SELECT * FROM tbl_refs_is_fisherfolk WHERE trans_id='" & _id & "'")
